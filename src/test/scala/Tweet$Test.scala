...
class Tweet$Test extends FunSuite {

  test("testApply") {
    val tweet: Tweet = Tweet(jsonString)

    assert(tweet.tweetID == 54691802283900930L)
    assert(tweet.authorID == 80177619L)
    assert(tweet.hashtags.equals(Set()))
    assert(tweet.mentionIDS.equals(Set(271572434)))

    assert(tweet.retweetOf.nonEmpty)

    val retweet = tweet.retweetOf.get
    assert(retweet.tweetID == 54640519019642880L)
    assert(retweet.authorID == 271572434L)
    assert(retweet.retweetOf.isEmpty)
    assert(retweet.hashtags.equals(Set("PGP")))
    assert(retweet.mentionIDS.equals(Set()))
  }
